 ...
 * registration per message type — and on 1.20.5+, where each needs a payload class and a
 * codec, that multiplies. Folding the name into the body keeps both versions to a single
        ...
        server.execute(() -> {
            try {
                ...
                BlockiesEconomy.LOGGER.warn("Bad '{}' packet from {}: {}",
                        name, player.getGameProfile().getName(), e.toString());
            }
        });
    }
